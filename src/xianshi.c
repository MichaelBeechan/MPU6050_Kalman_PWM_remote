...
char  code C_0 = 1;
float xdata Q_bias=0, Angle_err=0;
...
void Angle_Calcu(void)	 
{
	...
	Angle_ax = Accel_x/16384;
	Angle_ax = Angle_ax*1.5*180/3.14;     //弧度转换为度,

	Display10BitData(Angle_ax,8,1);		 

	 Accel_y  = GetData(ACCEL_YOUT_H);
	 Angle_ay = Accel_y/16384;
	 Angle_ay = -Angle_ay*1.5*180/3.14;
	 Display10BitData(Angle_ay,8,2);		
    //-------角速度-------------------------

	//范围为2000deg/s时，换算关系：16.4 LSB/(deg/s)
  
	Gyro_y = GetData(GYRO_YOUT_H);	      //静止时角速度Y轴输出为-30左右
	Gyro_y = -(Gyro_y + 30)/16.4;         //去除零点偏移，计算角速度值,负=号为方向处理 
	//Angle_gy = Angle_gy + Gyro_y*0.01;  //角速度积分得到倾斜角度.
    //Display10BitData(Angle_gy,8,2);	  	   
